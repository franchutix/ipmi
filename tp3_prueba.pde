...
void draw() {
  ...
  for (int columna = 0; columna < numColumnas; columna++) {
    for (int fila = 0; fila < numFilas; fila++) {
      ...
      int nuevoAncho = calcularTamañoForma(formaAncho, factor);
      int nuevoAlto = calcularTamañoForma(formaSize * 5, factor);
...
// Nueva función que retorna un valor
int calcularTamañoForma(int baseTamaño, float factorZoom) {
  return int(baseTamaño * factorZoom);
}
